...
class Layer1Train():
    ...
    def make_oof(self, model, X_train, y_train, X_test, MODEL_NAME):
        ...
        n_seed = len(seed)
        for n, v in enumerate(seed):
            print('Iter {}  Seed for KFold is : {}'.format(n + 1, v))
            skf = KFold(n_splits=KFOLDS, random_state=v, shuffle=True)
            score = []
            for i, (train_index, val_index) in enumerate(skf.split(X_train, y_train)):
                ...
                oof_train[val_index] += (pred_val / n_seed)
                # Store predictions for submission and oof test
                oof_test += (pred_test / n_seed)
            ...
            seed_score.append(avg_score)
            print('Average score: {}'.format(avg_score))

        print('\n')
        print('All seed score: {}'.format(seed_score))
        print('Average score for All seed: {}'.format(np.mean(seed_score)))
        # Export oof_train
